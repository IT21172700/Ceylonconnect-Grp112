...
const UserInquiries = () => {
  ...
  const fetchInquiries = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/inquiry/inquiries/${userId}`
      );
      const { data } = response;
      setInquiries(data);
    } catch (error) {
  ...
  const deleteInquiry = async (inquiryId) => {
    try {
      await axios.delete(
        `http://localhost:8080/api/inquiry/inquiries/${inquiryId}`
      );
  ...
  return (
    <div>
      ...
      <div style={{padding: '1rem', display: 'flex', justifyContent: 'end'}}>
        <Link to="/inquiry">
          <button class="btn btn-primary">Add New Inquiry</button>
        </Link>
      </div>
                ...
                <tr key={inquiry._id}>
                  <td>
                    {editingId === inquiry._id ? (
                      <input type="text" value={editData.packageId} disabled />
                    ) : (
                      inquiry.packageId
                    )}
                  </td>
                    ...
                    {editingId === inquiry._id ? (
                      <div class="btn-group">
                        <button
                          ...
                          onClick={() => updateInquiry(inquiry._id)}
                        >
                          Save
                        </button>
